...
El código del servidor DEBE residir en la carpeta `backend/`.
...
El código del cliente DEBE residir en la carpeta `frontend/`.
...
Los cambios DEBEN mantener el funcionamiento actual del sistema sin rupturas.
...
Las rutas de importación DEBEN actualizarse para reflejar la nueva estructura.
...
Los scripts de npm DEBEN actualizarse para funcionar con la nueva estructura.
...
La estructura de base hexagonal del backend DEBE mantenerse en `backend/`.
...
La estructura modular del frontend DEBE mantenerse en `frontend/src/modules/`.
